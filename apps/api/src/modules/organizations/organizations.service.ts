...
export class OrganizationsService {
  ...
  async getCurrent(user: AuthenticatedUser) {
    ...
    const org = await this.prisma.organization.findUnique({
      where: { id: orgId },
      include: {
        billingAccount: { select: { plan: true, status: true, seatsLimit: true, currentPeriodEnd: true } },
  ...
  async updateCurrent(user: AuthenticatedUser, data: { name?: string; legalName?: string; taxId?: string; branding?: unknown }) {
    const orgId = user.org?.id;
    if (!orgId) throw new ForbiddenException('No active tenant');
  ...
  async listWorkspaces(user: AuthenticatedUser) {
    const orgId = user.org?.id!;
    return this.prisma.workspace.findMany({ where: { organizationId: orgId }, orderBy: { createdAt: 'asc' } });
  }
  ...
  async createWorkspace(
    ...
    return this.prisma.workspace.create({
      data: { organizationId: orgId, name: data.name, slug, description: data.description, visibility: data.visibility, createdById: user.id },
    });
  }
  ...
  async createTeam(user: AuthenticatedUser, data: { name: string; description?: string; workspaceId?: string; memberIds?: string[] }) {
    const orgId = user.org?.id!;
    this.assertManager(user);
    const team = await this.prisma.team.create({
      data: { organizationId: orgId, workspaceId: data.workspaceId, name: data.name, description: data.description },
    });
