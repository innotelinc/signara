...
| API for integrators                            | `api-keys` module, `openapi/`; outbound webhooks (`webhooks` module, `docs/Webhooks.md`) — **landed 2026-09-20** (#85)                  | webhook deliveries are queued and retried, but nothing consumes `WebhookDelivery` to auto-disable an endpoint that has failed for weeks       |
